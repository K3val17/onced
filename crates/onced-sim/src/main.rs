...
use onced_sim::{Simulation, Stats};

fn main() {
    let seeds = env_u64("ONCED_SIM_SEEDS", 200);
    let steps = env_u64("ONCED_SIM_STEPS", 2_000);

    eprintln!("onced-sim: {seeds} seeds x {steps} steps of fault-injected operations...");
    ...
    println!("All invariants held across {seeds} seeds x {steps} steps.");
    println!("{total:#?}");
}
